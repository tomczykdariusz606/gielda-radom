...
# Ustawiamy ścieżkę do folderu instance, aby backupy działały poprawnie
...
def get_market_valuation(car):
    ...
    if diff_percent < -15: return {"status": "SUPER OKAZJA", "pos": 20, "color": "#28a745", "diff": round(diff_percent, 1), "avg": int(estimated_avg)}
    elif diff_percent < 5: return {"status": "CENA RYNKOWA", "pos": 50, "color": "#1a73e8", "diff": round(diff_percent, 1), "avg": int(estimated_avg)}
    else: return {"status": "POWYŻEJ ŚREDNIEJ", "pos": 80, "color": "#ce2b37", "diff": round(diff_percent, 1), "avg": int(estimated_avg)}
...
def dodaj_ogloszenie():
    ...
    paths = [save_optimized_image(f) for f in files[:10] if f and allowed_file(f.filename)]
    main_img = paths[0] if paths else 'https://placehold.co/600x400?text=Brak+Zdjecia'
    
    ai_analysis = ""
    if paths:
        try:
            img_path = os.path.join(app.root_path, paths[0].lstrip('/'))
            vision_res = model_ai.generate_content(["Opisz krótko stan wizualny tego auta po polsku.", Image.open(img_path)])
    ...
    nowe_auto = Car(
        ...
        opis=request.form['opis'] + ai_analysis, telefon=request.form['telefon'],
    ...
    db.session.flush()
    for p in paths: db.session.add(CarImage(image_path=p, car_id=nowe_auto.id))
    db.session.commit()
...
def robots():
    lines = ["User-agent: *", "Allow: /", "Sitemap: https://gieldaradom.pl/sitemap.xml"]
