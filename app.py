...
# 1. Inicjalizacja zmiennych środowiskowych (MUSI BYĆ TUTAJ)
load_dotenv()
...
app.secret_key = os.getenv("SECRET_KEY", "sekretny_klucz_gieldy_radom_2026")
...
if GEMINI_KEY:
    ...
    print("⚠️ BŁĄD: Brak klucza API w pliku .env! Funkcje AI nie będą działać.")
...
def generate_ai_description():
    if not vision_model: return jsonify({"error": "AI niedostępne"}), 503
    data = request.json
    prompt = f"Napisz profesjonalne ogłoszenie dla {data.get('marka')} {data.get('model')} ({data.get('rok')}, {data.get('paliwo')}) w Radomiu. 4-5 zdań, profesjonalny styl."
    try:
        response = vision_model.generate_content(prompt)
        return jsonify({"description": response.text.strip()})
    except Exception as e:
        return jsonify({"description": f"Sprzedam {data.get('marka')} {data.get('model')}. Stan dobry!"})
...
def analyze_car():
    ...
    try:
        ...
        prompt = "Zwróć TYLKO czysty JSON: {\"marka\": \"...\", \"model\": \"...\", \"rok\": 2020}. Rozpoznaj auto ze zdjęcia."
        response = vision_model.generate_content([prompt, {"mime_type": "image/jpeg", "data": img_b64}])
        
        match = re.search(r'\{.*\}', response.text.strip(), re.DOTALL)
        if match:
            return jsonify(json.loads(match.group()))
        return jsonify({"error": "Nie rozpoznano auta"})
    except Exception as e:
...
def register():
    if request.method == 'POST':
        ...
        u = User(username=request.form['username'], email=request.form['email'], password_hash=hashed)
        db.session.add(u)
        db.session.commit()
