...
class Car(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    typ = db.Column(db.String(20), default='Osobowe') # Ważne: to pole musi być w bazie!
...
# --- SEKRETNY PANEL ADMINISTRATORA I BACKUPY ---
...
def backup_db():
    ...
    return send_file(db_path, as_attachment=True, download_name=f"baza_{datetime.now().strftime('%Y%m%d')}.db")
...
def full_backup():
    ...
    memory_file.seek(0)
    return send_file(memory_file, mimetype='application/zip', as_attachment=True, download_name="FULL_BACKUP_SYSTEMU.zip")
...
def sitemap():
    ...
    for c in cars: xml += f'<url><loc>{base_url}/ogloszenie/{c.id}</loc><priority>0.8</priority></url>\n'
    xml += '</urlset>'
...
def profil():
    ...
    return render_template('profil.html', cars=my_cars, fav_cars=current_user.favorite_cars, stats=stats, now=datetime.utcnow())
...
def dodaj_ogloszenie():
    ...
    # Automatyczna analiza AI przy dodawaniu
    ai_comment = ""
    if saved:
        try:
            full_p = os.path.join(app.root_path, saved[0].lstrip('/'))
            res = model_ai.generate_content(["Krótko opisz stan auta ze zdjęcia (jedno zdanie).", Image.open(full_p)])
            ai_comment = f"\n\n[Analiza AI]: {res.text}"
        except: pass
...
def register():
    if request.method == 'POST':
        ...
        new_u = User(username=request.form['username'], email=request.form['email'], password_hash=hashed)
